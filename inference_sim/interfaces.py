...
class Perception(Protocol):
    ...
    def make_clean_image(
        ...
        objs: Optional[Dict[str, "ObjectState"]] = None,
    ) -> np.ndarray:
        """Per-step. Produce the embodiment-agnostic RGB the model expects:
        real arm inpainted away, a virtual gripper (and, if `objs` is given,
        per-object keypoints) rendered at each EE pose. Returns (H, W, 3) BGR
        uint8. If your model was trained 'state-only' (no image), this can
        return a black image and will be ignored."""
        ...
